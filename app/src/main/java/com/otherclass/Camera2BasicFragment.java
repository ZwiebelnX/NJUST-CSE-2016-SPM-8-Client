...
public class Camera2BasicFragment extends Fragment
    ...
    private final ImageReader.OnImageAvailableListener mOnImageAvailableListener
        ...
        public void onImageAvailable(ImageReader reader) {
            ...
            buffer.get(data);
            Log.i("getimage","获取到图片获取到图片"+img.getHeight()+" "+img.getWidth());
    ...
    private void commuInfo(byte[] bytes){
        ...
        if(null !=bytes) {
            ...
            try{
                ...
                MediaType type=MediaType.parse("application/octet-stream");
                RequestBody fileBody=RequestBody.create(type,bytes);
                Log.d("chuanguoqudedongxi",mcourseID+" " + mtype+ " 图片：" +bytes.length);
                //times+=1;
            ...
            public void handleMessage(Message msg){
                ...
                try{
                    ...
                    if(checkResult!=null&&!checkResult.equals("SUCCESS")){
                        info = checkResult;//"服务器或传输错"
                    }else{
                        checkpercent = json.getString("result");
                        mCNT=Integer.parseInt(json.getString("checkCNT"));
                        JSONArray array=(JSONArray) json.get("studentList");
                        checkstudents = new ArrayList<>();
                        for(int i=0;i<array.length();i++){
                            JSONObject obj = (JSONObject)array.get(i);
                            Check_Student stu=new Check_Student(obj.getString("studentName"),obj.getString("studentID"),obj.getString("checkResult"));

                            checkstudents.add(stu);
                        }
                        Log.i("letmesecs","让我看看有没有cs");
